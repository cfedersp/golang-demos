...
/*
This method needs work, but the potential for a strong solution is there.
 */
func backpressureManager(initialAdvice int, bufferLength int, statusChannel chan int, adviceChannel chan int) {
	adviceChannel <- initialAdvice;
	lastAdvice := initialAdvice;
	isAboveThreshold := true;

	rollingStatusWindow := make([]int, bufferLength)

	retryThreshold := 0.5;
	retriesInLastRollingWindow := float64(0);

	for {
		lastStatus := <- statusChannel;

		if(lastStatus == 429) {
			retriesInLastRollingWindow++
		}
		if(rollingStatusWindow[bufferLength-1] == 429) {
			retriesInLastRollingWindow--;
		}
		rollingStatusWindow = append([]int{lastStatus}, rollingStatusWindow...)

		if(isAboveThreshold && retriesInLastRollingWindow/float64(bufferLength) > retryThreshold) {
			lastAdvice--;
			isAboveThreshold = false;
			adviceChannel <- lastAdvice;
			fmt.Println("decreasing workers");
		} else if(!isAboveThreshold && retriesInLastRollingWindow/float64(bufferLength) < retryThreshold) {
			lastAdvice++;
			isAboveThreshold = true;
			adviceChannel <- lastAdvice;
...
func saveToOutputBucket(outputChannel chan CandidateRecommendation, statusChannel chan int) {
	for {
		select {
		case currCandidate := <-outputChannel:
			statusChannel <- currCandidate.status;
...
func main() {
	...
	outputDataChannel := make(chan CandidateRecommendation);
	responseStatusChannel := make(chan int);
	...
	go controller(mainChannel, priorityChannel, outputDataChannel, adviceChannel);
	go saveToOutputBucket(outputDataChannel, responseStatusChannel);
	go backpressureManager(concurrency, sensitivity, responseStatusChannel, adviceChannel);
